...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    SQLite does not support PostgreSQL's ``FOR UPDATE SKIP LOCKED``, so the
    SQLite path uses a ``BEGIN IMMEDIATE`` writer reservation to serialize
    claims across API processes.  On PostgreSQL a regular transaction is
    sufficient because claim-time row locking (``FOR UPDATE SKIP LOCKED`` in
    :func:`storage.claim_one`) coordinates concurrent workers.
    """
